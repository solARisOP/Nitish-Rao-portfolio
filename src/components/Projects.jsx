import { useNavigate } from "react-router-dom"
import { PROJECTS } from "../constants"
import { motion } from 'framer-motion'

function Projects() {
    const navigate = useNavigate()

    const gotoProject = (e) => {
        const idx = e.currentTarget.dataset.idx
        navigate(`/project/${idx}`)
    }

  return (
    <div className="border-b border-neutral-900 pb-4">
        <motion.h2 whileInView={{opacity: 1, y: 0}}
        initial={{opacity: 0, y: -100}}
        transition={{duration: 0.5}} className="my-20 text-center text-4xl">
            Projects
        </motion.h2>
        <div>
            {PROJECTS.map((project, idx) => 
                <motion.div onClick={gotoProject} data-idx={idx} key={idx} className="mb-8 flex flex-wrap lg:justify-center cursor-pointer rounded-xl" 
                whileHover={{
                    scale: 1.1
                  }}
                  transition={{
                    duration: 0.3
                  }}
                >
                    <motion.div whileInView={{opacity: 1, x:0}}
                    initial={{opacity: 0, x: -100}}
                    transition={{duration: 1}} className="w-full lg:w-1/4">
                        <img src={project.image} width={150} height={150} alt={project.title} className="mb-6 rounded" />
                    </motion.div>
                    <motion.div whileInView={{opacity: 1, x:0}}
                    initial={{opacity: 0, x: 100}}
                    transition={{duration: 1}} className="w-full max-w-xl lg:w-3/4">
                        <h6 className="mb-2 font-semibold">
                            {project.title}
                        </h6>
                        <p className="mb-4 text-neutral-400">
                            {project.subtitle}
                        </p>
                        <div className="flex justify-start items-center flex-wrap gap-3">
                            {project.technologies.map((tech, idx)=>
                                <span key={idx} className="rounded bg-neutral-900 px-2 py-1 text-sm font-medium text-purple-900"> 
                                    {tech}
                                </span>
                            )}
                        </div>
                    </motion.div>
                </motion.div>
            )}
        </div>
    </div>
  )
}

export default Projects